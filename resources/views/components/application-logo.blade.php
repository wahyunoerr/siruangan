<svg id="auth-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" {{ $attributes }}>
    <path
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const hasAuthFailed = @json($errors->has('email'));
        const hasPasswordError = @json($errors->has('password'));

        if (hasAuthFailed) {
            authIcon.querySelector('path').setAttribute(
                'd',
